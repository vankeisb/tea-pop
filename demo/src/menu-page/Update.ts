...
import { Cmd, just, Maybe, noCmd, nothing, Sub, Tuple } from "tea-cup-fp";
import {
  open as menuOpen,
...
export function menuPageUpdate(
  msg: MenuPageMsg,
  menuPage: MenuPage
): [MenuPage, Cmd<MenuPageMsg>] {
  switch (msg.tag) {
    case "menu-msg": {
    ...
    case "mouse-down": {
      ...
      if (msg.button === 2) {
        return updateMenu(
          menuPage,
          menuOpen(myMenu, box(menuPage.mousePos, Dim.zero))
        );
      }
...
export function menuPageSubs(page: MenuPage): Sub<MenuPageMsg> {
  ...
  // mouse & key subs
  const mouseMove: Sub<MenuPageMsg> = documentEvents.on("mousemove", (e) =>
    onMouseMove(pos(e.clientX, e.clientY))
  );
  const keyDown: Sub<MenuPageMsg> = documentEvents.on("keydown", (e) =>
    onKeyDown(e.key)
  );
...
function closeMenu(
  menuPage: MenuPage,
  lastClicked: Maybe<string> = nothing
): [MenuPage, Cmd<MenuPageMsg>] {
  return noCmd({
    ...menuPage,
    menuModel: nothing,
