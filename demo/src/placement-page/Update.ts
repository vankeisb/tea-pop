...
export function placementPageUpdate(
  msg: PlacementPageMsg,
  page: PlacementPage
): [PlacementPage, Cmd<PlacementPageMsg>] {
  switch (msg.tag) {
...
export function placementPageSubs(): Sub<PlacementPageMsg> {
  const mouseMove: Sub<PlacementPageMsg> = documentEvents.on("mousemove", (e) =>
    onMouseMove(pos(e.pageX, e.pageY))
  );
  const keyDown: Sub<PlacementPageMsg> = documentEvents.on("keydown", (e) =>
    onKeyDown(e.key)
  );
  return Sub.batch([
    mouseMove,
    keyDown,
    windowEvents.on("resize", (e) =>
