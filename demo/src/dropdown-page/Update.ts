...
import { Cmd, just, noCmd, nothing, Sub, Task, Tuple } from "tea-cup-fp";
import {
  DropDownModel,
  DropDownMsg,
  dropDownOpen,
...
export function dropDownPageUpdate(
  msg: DropDownPageMsg,
  ddPage: DropDownPage
): [DropDownPage, Cmd<DropDownPageMsg>] {
  switch (msg.tag) {
    case "button-clicked": {
      const ddMac: [DropDownModel, Cmd<DropDownMsg>] = dropDownOpen(
        Task.succeed(msg.b)
      );
      ...
      return Tuple.t2n(newPage, ddMac[1].map(ddMsg));
    }
    case "dd-msg": {
      return ddPage.indexAndModel
        .map((iam) => {
          ...
          const res: [DropDownPage, Cmd<DropDownPageMsg>] = Tuple.fromNative([
            ddMac[0],
            ddMac[1],
          ])
            .mapFirst((ddModel) => ({
              ...ddPage,
              indexAndModel: ddPage.indexAndModel.map(
                (t) => new Tuple(t.a, ddModel)
              ),
            }))
